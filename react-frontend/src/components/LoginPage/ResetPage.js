...
const PasswordPolicy = ({ text, valid }) => {
    return (
        <p className={classNames('flex items-center gap-2 mb-1', !valid && 'text-[#ADB5BD]')}>
            <span className="w-[15px] text-center">
                <i className={classNames(valid ? 'text-green-500 pi pi-check' : 'text-[#ADB5BD] pi pi-circle-fill text-xs')}></i>
            </span>
            {text}
        </p>
    );
    ...
    return (
        <div className="flex flex-col min-h-screen align-items-center justify-content-center background-image">
                        ...
                        <div className="mt-6">
                            <div className="w-full mb-4">
                                ...
                                <span className="w-full p-input-icon-right">
                                    <i className={`pi ${maskPassword ? 'pi-eye' : 'pi-eye-slash'} cursor-pointer`} title={`${maskPassword ? 'Show' : 'Hide'} password`} onMouseDown={() => setMaskPassword(false)} onMouseUp={() => setMaskPassword(true)} onMouseLeave={() => setMaskPassword(true)} />
                                    <InputText
                                ...
                                <div className="mt-2">
                                    <PasswordPolicy text="Minimum of 10 characters" valid={isMinLength} />
                                    <PasswordPolicy text="Include at least one uppercase letter" valid={isUppercase} />
                                    <PasswordPolicy text="Include at least one number" valid={isNumber} />
                                    <PasswordPolicy text="Include at least one symbol" valid={isSymbol} />
                                </div>
                            </div>
                            <div className="w-full mb-4">
                                ...
                                <span className="w-full p-input-icon-right">
                                    ...
                                    <InputText
                                        ...
                                        className={classNames(confirmPasswordError ? 'p-invalid' : '', 'w-full')}
                                        onKeyDown={onEnter}
                                    ></InputText>
                                </span>
                            ...
                            <div className="mt-6">
                                <Button label="Reset password" className="w-full !rounded-full py-3 text-[16px]" onClick={savePassword}></Button>
                            </div>
                        </div>
                    </div>
                )}
                {success && (
                    <div className="card w-full max-w-[500px] px-3 md:px-7 mx-3">
                        <div className="my-4 text-center">
                            <img src={successImage} alt="success" className="w-[60%] min-w-[150px]" />
                        </div>
                        <div className="text-center">
                            <h2 className="mb-4 text-2xl font-semibold">Successfully reset your password</h2>
                            <p className="text-[14px]">Please log in with your new password</p>
                        </div>
                        <div className="mt-6">
                            <Button
                                label="Back to login"
            ...
            <div className="absolute bottom-0 left-0 flex justify-between w-full px-10 py-10 z-[-1]">
                <img src={illustration1} alt="illustration1" className="w-[20%] min-w-[150px]" />
                <img src={illustration2} alt="illustration2" className="w-[20%] min-w-[150px]" />
            </div>
            <AppFooter />
        </div>
    );
