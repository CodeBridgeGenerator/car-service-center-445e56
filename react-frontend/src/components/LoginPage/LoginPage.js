...
const LoginPage = (props) => {
    ...
    const login = async () => {
        if (validate()) {
            try {
                setLoading(true);
                await props.login({ email, password }).then(async (res) => {
                    if (!res.user._id) throw Error('Invalid login');
                    else
                        try {
                            // Save login history with additional details
                            const ipAddress = (await getIPAddress()) ?? null;
