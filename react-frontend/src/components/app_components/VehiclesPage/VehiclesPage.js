...
import VehiclesDatatable from "./VehiclesDataTable";
import VehiclesEditDialogComponent from "./VehiclesEditDialogComponent";
import VehiclesCreateDialogComponent from "./VehiclesCreateDialogComponent";
import VehiclesFakerDialogComponent from "./VehiclesFakerDialogComponent";
import VehiclesSeederDialogComponent from "./VehiclesSeederDialogComponent";
...
const VehiclesPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("vehicles");
    ...
    useEffect(() => {
        ...
        client
            .service("vehicles")
            .find({ query: { $limit: 10000 , customerId : urlParams.singleCustomersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("vehicles").remove(selectedEntityIndex);
            ...
            console.debug({ error });
            props.alert({ title: "Vehicles", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("vehicles").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="vehicles-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="vehicles-datatable">
                    <VehiclesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <VehiclesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <VehiclesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <VehiclesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <VehiclesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(VehiclesPage);
