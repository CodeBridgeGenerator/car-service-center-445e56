...
import vehiclesFakerFactory from "./vehiclesFakerFactory";
...
const VehiclesFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("customers")
                        .find({ query: { $select: ["_id","firstName"] } })
                        .then((data) => {
    ...
    const onRun = async () => {
        let fakeData = vehiclesFakerFactory(props.user,fakerCount,customerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vehicles").create(elem));
...
export default connect(mapState, mapDispatch)(VehiclesFakerDialogComponent);
