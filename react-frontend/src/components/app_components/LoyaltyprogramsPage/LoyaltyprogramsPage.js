...
import LoyaltyprogramsDatatable from "./LoyaltyprogramsDataTable";
import LoyaltyprogramsEditDialogComponent from "./LoyaltyprogramsEditDialogComponent";
import LoyaltyprogramsCreateDialogComponent from "./LoyaltyprogramsCreateDialogComponent";
import LoyaltyprogramsFakerDialogComponent from "./LoyaltyprogramsFakerDialogComponent";
import LoyaltyprogramsSeederDialogComponent from "./LoyaltyprogramsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const LoyaltyprogramsPage = (props) => {
    ...
    const filename = "loyaltyprograms.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("loyaltyprograms");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("loyaltyprograms").remove(selectedEntityIndex);
            ...
            console.debug({ error });
            props.alert({ title: "Loyaltyprograms", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("loyaltyprograms").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/loyaltyprograms/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="loyaltyprograms-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="loyaltyprograms-datatable">
                    <LoyaltyprogramsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <LoyaltyprogramsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <LoyaltyprogramsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <LoyaltyprogramsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <LoyaltyprogramsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(LoyaltyprogramsPage);
