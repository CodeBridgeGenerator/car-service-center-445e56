...
import loyaltyprogramsFakerFactory from "./loyaltyprogramsFakerFactory";
...
const LoyaltyprogramsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = loyaltyprogramsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("loyaltyprograms").create(elem));
...
export default connect(mapState, mapDispatch)(LoyaltyprogramsFakerDialogComponent);
