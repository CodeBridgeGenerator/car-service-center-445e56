...
const PaymentmethodsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.methodName}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.description}</p>
  ...
  const handleHideDialog = () => {
    ...
    return (
        ...
        />
<Column field="methodName" header="MethodName" body={pTemplate0} filter={selectedFilterFields.includes("methodName")} hidden={selectedHideFields?.includes("methodName")}  sortable style={{ minWidth: "8rem" }} />
<Column field="description" header="Description" body={pTemplate1} filter={selectedFilterFields.includes("description")} hidden={selectedHideFields?.includes("description")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Paymentmethods Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="paymentmethods"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search Paymentmethods" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
