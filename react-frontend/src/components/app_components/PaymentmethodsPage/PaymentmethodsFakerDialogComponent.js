...
import paymentmethodsFakerFactory from "./paymentmethodsFakerFactory";
...
const PaymentmethodsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = paymentmethodsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("paymentmethods").create(elem));
...
export default connect(mapState, mapDispatch)(PaymentmethodsFakerDialogComponent);
