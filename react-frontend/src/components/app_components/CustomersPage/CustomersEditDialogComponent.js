...
const [phoneNumber, setPhoneNumber] = useState([])
 ...
 useEffect(() => {
    ...
    const onSave = async () => {
        let _data = {
            firstName: _entity?.firstName?._id,
lastName: _entity?.lastName,
email: _entity?.email,
phoneNumber: _entity?.phoneNumber?._id,
address: _entity?.address,
joinDate: _entity?.joinDate,
loyaltyPoints: _entity?.loyaltyPoints,
        };

        setLoading(true);
        try {
            
        await client.service("customers").patch(_entity._id, _data);
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="joinDate" className="w-full mb-3 p-inputtext-sm" value={_entity?.joinDate} onChange={(e) => setValByKey("joinDate", e.value)}  useGrouping={false}/>
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="loyaltyPoints" className="w-full mb-3 p-inputtext-sm" value={_entity?.loyaltyPoints} onChange={(e) => setValByKey("loyaltyPoints", e.value)}  useGrouping={false}/>
            </span>
            <small className="p-error">
