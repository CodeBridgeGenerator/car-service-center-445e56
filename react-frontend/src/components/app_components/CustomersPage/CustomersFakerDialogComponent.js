...
import customersFakerFactory from "./customersFakerFactory";
...
const CustomersFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [firstNameIds, setfirstNameIds] = useState();
const [phoneNumberIds, setphoneNumberIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("users")
                        .find({ query: { $select: ["_id","password"] } })
                        .then((data) => {
                        setfirstNameIds(data.data);
                        });
client.service("userPhones")
                        .find({ query: { $select: ["_id","number"] } })
                        .then((data) => {
                        setphoneNumberIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = customersFakerFactory(props.user,fakerCount,firstNameIds,phoneNumberIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customers").create(elem));
...
export default connect(mapState, mapDispatch)(CustomersFakerDialogComponent);
