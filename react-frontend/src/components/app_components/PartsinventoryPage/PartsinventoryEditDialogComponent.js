...
supplierId: _entity?.supplierId,
        ...
        const result = await client.service("partsinventory").patch(_entity._id, _data);
        props.onHide();
    ...
    return (
        <Dialog header="Edit Parts Inventory" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="supplierId" className="w-full mb-3 p-inputtext-sm" value={_entity?.supplierId} onChange={(e) => setValByKey("supplierId", e.value)}  useGrouping={false}/>
            </span>
            <small className="p-error">
