...
const PartsinventoryCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            partName: _entity?.partName,description: _entity?.description,quantityInStock: _entity?.quantityInStock,price: _entity?.price,supplierId: _entity?.supplierId,
        ...
        const result = await client.service("partsinventory").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Parts Inventory created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Parts Inventory" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Parts Inventory" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="partsinventory-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="supplierId" className="w-full mb-3 p-inputtext-sm" value={_entity?.supplierId} onChange={(e) => setValByKey("supplierId", e.value)}  />
            </span>
            <small className="p-error">
