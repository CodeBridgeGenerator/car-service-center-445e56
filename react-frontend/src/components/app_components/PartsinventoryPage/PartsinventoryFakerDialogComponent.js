...
import partsinventoryFakerFactory from "./partsinventoryFakerFactory";
...
const PartsinventoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = partsinventoryFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("partsinventory").create(elem));
...
export default connect(mapState, mapDispatch)(PartsinventoryFakerDialogComponent);
