...
import oilchangerecordsFakerFactory from "./oilchangerecordsFakerFactory";
...
const OilchangerecordsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = oilchangerecordsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("oilchangerecords").create(elem));
...
export default connect(mapState, mapDispatch)(OilchangerecordsFakerDialogComponent);
