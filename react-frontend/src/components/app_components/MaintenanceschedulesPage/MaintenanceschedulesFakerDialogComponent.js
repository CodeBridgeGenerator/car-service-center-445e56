...
import maintenanceschedulesFakerFactory from "./maintenanceschedulesFakerFactory";
...
const MaintenanceschedulesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = maintenanceschedulesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("maintenanceschedules").create(elem));
...
export default connect(mapState, mapDispatch)(MaintenanceschedulesFakerDialogComponent);
