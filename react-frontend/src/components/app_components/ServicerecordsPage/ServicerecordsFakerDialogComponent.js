...
import servicerecordsFakerFactory from "./servicerecordsFakerFactory";
...
const ServicerecordsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = servicerecordsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("servicerecords").create(elem));
...
export default connect(mapState, mapDispatch)(ServicerecordsFakerDialogComponent);
