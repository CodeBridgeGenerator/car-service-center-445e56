...
const [vehicleIdIds, setvehicleIdIds] = useState();
...
client.service("vehicles")
                        .find({ query: { $select: ["_id","make"] } })
                        .then((data) => {
                        setvehicleIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = invoicesFakerFactory(props.user,fakerCount,customerIdIds,vehicleIdIds);
